...
public class JsonGameDaoTest {
    ...
    public void givenSnapshotSaved_whenReadWithNewDaoInstance_thenDataSurvives() {
        Path file = tempFolder.getRoot().toPath().resolve("games.json");
        JsonGameDao writer = new JsonGameDao(file);
        ...
        JsonGameDao reader = new JsonGameDao(file);
    ...
    public void givenSnapshotSaved_whenSavedAgainWithSameId_thenReplacesInsteadOfDuplicating() {
        Path file = tempFolder.getRoot().toPath().resolve("games.json");
        JsonGameDao dao = new JsonGameDao(file);

        dao.save(minimalSnapshot(1));
        dao.save(minimalSnapshot(1));
